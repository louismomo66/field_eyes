...
docker-compose -f docker-compose.full.yml down &>/dev/null || true
docker rm -f fieldeyes-postgres fieldeyes-redis fieldeyes-api &>/dev/null || true

# Start the full stack with docker-compose
echo -e "${BLUE}Starting the full application stack with Docker Compose...${NC}"
...
echo -e "  ${GREEN}docker logs fieldeyes-postgres${NC}    # View PostgreSQL logs"
echo -e "  ${GREEN}docker logs fieldeyes-redis${NC}    # View Redis logs"

# Show how to stop the stack
echo -e "\n${BLUE}To stop the stack:${NC}"
echo -e "  ${GREEN}docker-compose -f docker-compose.full.yml down${NC}" 